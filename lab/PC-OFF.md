...
2. Connect GitHub to Pitchfork-and-Torch if not already
3. Start Cloud agents on `vela` for JON-11 then JON-12 (Grok model if listed)
4. Comment `@cursor` on the GitHub issues if Cloud is wired to GitHub
5. Watch Telegram @KnockNGrok_bot for VPS kicks
6. Review PRs on the phone. Do not merge write-enabled CCA composes.
