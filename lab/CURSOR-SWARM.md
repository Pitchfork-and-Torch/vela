...
Implement one LANGUAGE.md gap per session. Interval n>=2, compose cuts/growth, harness INCOMPLETE, observe posture, hint law, typed reconfig, and passthrough already landed (PRs 4/5/6/7/8/9/10). Closed-write compose still needs `posture review`. Observe `when`/`every` cannot write pace/cwnd. Tests must stay green (`py -3 -m unittest discover -s tests -q`). No packet-path writes.
