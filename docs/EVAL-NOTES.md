...
**typed-loss / unknown-fail-closed:** `vela check examples/reach.vela` stamps `loss=Mobility|Congestive|Unknown` and `unknown=fail-closed` (delay_ratio<=1.35 refuses cut at check; fail-closed, not advisory). A bare Loss, a Mobility cut, or an Unknown cut without `delay_ratio > 1.35` is a type error on observe. SoftReprobe cut stays 0.58 (do not retune). Compile no longer stamps `typed_loss` for a bare handler. No packet-path change.
