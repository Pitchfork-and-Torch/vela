...
| Law | What it refuses |
|-----|-----------------|
| Level vs integrator | `when` / `every` `{ pace *= k }` without `integrate when` / `integrate every` (stamps `level_vs_integrator`) |
