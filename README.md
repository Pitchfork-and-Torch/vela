...
**v0.4.3 gate label:** `--fast` cannot be mixed with `--seeds`/`--duration`. Gate is stamped from the rows that ran. A worker writes the result row to a file so a later stdout line cannot replace it. ACCEPT on `gate=fast` is not a house dual-gate claim.
