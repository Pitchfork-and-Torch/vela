...
class TestTeaserSafety(unittest.TestCase):
    def test_readme_vela_fences_are_observe_passthrough(self):
        ...
        for i, src in enumerate(srcs):
            ...
            self.assertTrue(res.affine, i)
            self.assertTrue(res.hybrid, i)
            self.assertEqual(prog.controllers[0].name, "Reach", i)
            self.assertEqual(res.closed_writes, [])

    def test_readme_is_public_mit(self):
        text = README.read_text(encoding="utf-8")
        self.assertIn("github.com/Pitchfork-and-Torch/vela", text)
        self.assertNotIn("GitHub copy is **private**", text)
