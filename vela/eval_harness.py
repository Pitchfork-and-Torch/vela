...
def _leo_root() -> Path:
    return leo_aware_root()


def jain_index(throughputs: list[float]) -> float:
    """RFC 5166 Jain fairness. Local (no numpy). Empty is nan."""
...
def run_one(mod, factory: Callable, cfg, n_flows: int) -> Row:
    ...
    try:
        ...
        name = m.name
    thr = [float(x.goodput_bps) for x in metrics]
    jain = jain_index(thr) if n_flows > 1 else 1.0
    if n_flows > 1:
        gp = (sum(thr) / len(thr)) / 1e6
        p95 = (sum(float(x.p95_rtt_s) for x in metrics) / len(metrics)) * 1000
        avg = (sum(float(x.avg_rtt_s) for x in metrics) / len(metrics)) * 1000
        loss = sum(float(x.loss_rate) for x in metrics) / len(metrics)
    else:
        gp = m.goodput_bps / 1e6
...
def evaluate(
    ...
    *,
    ...
    summary["config"] = {
        ...
        "no_oracle": cfg.no_oracle,
        "jain_min": cfg.jain_min,
        "seeds": seeds,
        "scenarios": scenarios,
        "duration_s": duration_s,
...
def _required_asserts(cfg: VelaConfig) -> tuple[str, ...]:
    if getattr(cfg, "jain_min", None) is not None:
        return REQUIRED_ASSERTS + ("fairness_jain",)
    return REQUIRED_ASSERTS
...
def _decide_verdict(
    ...
    n_seeds: int,
    contract_min: int,
    required: tuple[str, ...] = REQUIRED_ASSERTS,
) -> str:
...
def _summarize(rows: list[dict], cfg: VelaConfig) -> dict:
    ...
    for (scen, cca), rs in sorted(by.items()):
        ...
        jains = [
            float(r["jain_fairness"])
            for r in rs
            if r.get("jain_fairness") is not None
        ]
        if jains and scen == FAIRNESS_SCENARIO:
            rec["jain_mean"] = round(_mean(jains), 4)
            rec["jain_std"] = round(_std(jains), 4)
        tables.append(rec)
