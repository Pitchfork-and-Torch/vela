...
class Parser:
        ...
        while not self.at("RBRACE"):
            if self.at("compose"):
                ...
                if self.at("when"):
                    ...
                    t = self.cur()
                    raise ParseError(
                        f"{self.name}:{t.line}:{t.col}: "
                        "integrate must precede when or every"
                    )
            elif self.at("when"):
    ...
    def _split(self) -> Split:
        ...
        if not self.at("into"):
            t = self.cur()
            raise ParseError(
                f"{self.name}:{t.line}:{t.col}: split requires `into`"
            )
        self.eat("into")
        ...
        while True:
            ...
            if self.at("COLON"):
                ...
                try:
                    weight = int(float(raw.rstrip("s")))
                except ValueError as exc:
                    raise ParseError(
                        f"{self.name}:{sp.line}:{sp.col}: split weight must be an integer"
                    ) from exc
        ...
        if len(children) < 2:
            raise ParseError(
                f"{self.name}:{sp.line}:{sp.col}: split needs at least two children"
            )
        return Split(parent=parent, children=children, span=sp)
    ...
    def _stmt(self) -> Stmt:
        ...
        if self.at("borrow"):
            ...
            name = self._name()
            body = self._block()
            return Stmt(kind="borrow", name=name, body=body, span=sp)
