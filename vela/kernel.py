...
class HorizonCCA:
    ...
    def _make_leo(self, **kw):
        ...
        def wrapped(t, reason, **okw):
            ...
            if entered and before_cwnd > 0:
                applied = float(cca.cwnd) / before_cwnd
                extras: list[float] = []
                if (
                    self.cfg.soft_flicker
                    ...
                    if (
                        ...
                        extras.append(float(self.cfg.soft_flicker_cut))
                # Soft cuts compose as min. SoftFlicker 0.85 cannot undo
                # SoftReprobe 0.58 (seed 7 45s dumped 55.2 / 123.8).
                cca.cwnd = apply_composed_cut(before_cwnd, [applied, *extras])
            if entered:
                why = str(reason)
                rtt_hit = any(
...
def leo_aware_root() -> Path:
    """Locate the sibling LeoAware sim. Override with LEO_AWARE_TRANSPORT."""
    import os

    env = (os.environ.get("LEO_AWARE_TRANSPORT") or "").strip()
    ...
    cands.extend(
        [
            Path.home() / "Projects" / "leo-aware-transport",
            here.parent / "leo-aware-transport",
            here.parent / "Projects" / "leo-aware-transport",
            Path.cwd() / "leo-aware-transport",
        ]
    )
    for p in cands:
        try:
            if p.is_dir() and (p / "leo_cc").is_dir():
                return p
        except OSError:
            continue
    return cands[0]
